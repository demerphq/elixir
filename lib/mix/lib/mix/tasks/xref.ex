defmodule Mix.Tasks.Xref do
  ...
  The following options are accepted:
    ...
    * `--format` - can be set to one of:
      ...
      * `json` *(since v1.18.4)* - produces a JSON file, by default written to
        ...
        sink files and whose values specify the type of relationship, which and will
    ...
    {found, count} =
      case opts[:format] do
        "dot" ->
          ...
          file_spec = Mix.Utils.write_dot_graph!("xref graph", Enum.sort(roots), callback, opts)

          if file_spec != "-" do
            png_file_spec = (file_spec |> Path.rootname() |> Path.basename()) <> ".png"

            location =
              if Path.dirname(file_spec) == "." and !String.starts_with?(file_spec, "."),
                do: " in the current directory",
                else: ""

            # maybe this would be better as a comment in the dot file.
            """
            Generated '#{inspect(file_spec)}'#{location}. To generate a PNG:
        ...
        "json" ->
          ...
          path = Mix.Utils.write_json_tree!(Enum.sort(roots), callback, opts)
